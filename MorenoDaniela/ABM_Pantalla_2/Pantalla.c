...
int ordenarArrayPorNombre (char arrayStr [][50], char *nombre, int limite)
{
    int i;
    int j;
    for (i=0;i<limite;i++)
...
int ordenarArrayPorPrecio (Pantalla* pPantalla, int limite)
{
    int i;
    int j;
    Pantalla buffer;
    for (i=0;i<limite-1;i++)
    {
        if (pPantalla[i].isEmpty==1)
        {
            ...
            for (j=i+1;j<limite;j++)
            {
                ...
                else if (pPantalla[i].precio==pPantalla[j].precio)
                {
...
int getById(Pantalla* array, int id, int limite, Pantalla* idEncontrado)
{
    int i;

    for (i=0;i<limite;i++)
    {
        if (id == array[i].idPantalla)
        {
            *idEncontrado=array[i];
            break;
        }
    }
    return 0;
